...
int main() {
    ...
    assert(result == PIPER_OK);
    assert(Ort::get_tensor_create_call_count() == 4);
    std::cout << "  4 input tensors created\n";

    // Check first tensor (phoneme_ids)
    const auto& tensors = Ort::Value::GetLastCreatedTensors();
    assert(tensors.size() >= 1);
    assert(tensors[0].tensor_type_ == Ort::ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64);
    std::cout << "  phoneme_ids tensor created with correct type\n";

    // Check scales tensor
    assert(tensors[2].tensor_type_ == Ort::ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT);
    ...
    std::cout << "\nTest 6: Mock output configuration\n";
    Ort::mock_session_set_single_output(100, nullptr, 0);

    result = piper_synthesize_next(synth, nullptr);
    assert(result == PIPER_OK);
    assert(Ort::get_run_call_count() == 1);
    assert(Ort::Session::last_session_->run_called == true);
    ...
    assert(Ort::get_run_call_count() == 3);
